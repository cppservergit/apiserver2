...
> **WARNING:** For security purposes, all the steps mentioned above should be executed on a separate machine not used to run APIServer2.
...
1. Copy `private.pem` and `secret.enc` into the APIServer2 directory.

2. Edit the `run.sh` script and change the value of `JWT_SECRET` like this:

export JWT_SECRET="secret.enc"
3. Save the file and restart the APIServer++ service.

## 7. Encrypt an ODBC Connection String

1. Save the connection string to a text file without a CRLF at the end.
...
3. Apply the new encrypted value. Copy the `.enc` file to the installation directory of APIServer2, then edit the `run.sh` script to change the value of the corresponding environment variable, like this:
